...
{ int j___0 ;
  ...
  j___0 = 0;
  while (j___0 < numf1s) {
    norm += *(f1_layer_P + j___0) * *(f1_layer_P + j___0);
    j___0 ++;
  }
  norm = sqrt(norm);
  norm *= c;
  ...
  j___0 = 0;
  while (j___0 < numf1s) {
    temp_sum = *(f1_layer_U + j___0) * *(f1_layer_U + j___0);
    norm += temp_sum;
    j___0 ++;
  }
  norm = sqrt(norm);
  ...
  j___0 = 0;
  while (j___0 < numf1s) {
    *(f1_layer_R + j___0) = (*(f1_layer_U + j___0) + c * *(f1_layer_P + j___0)) / sum;
    j___0 ++;
  }
  norm = (double )0;
  j___0 = 0;
  while (j___0 < numf1s) {
    norm += *(f1_layer_R + j___0) * *(f1_layer_R + j___0);
    j___0 ++;
  }
  norm = sqrt(norm);
  return (norm);
...
{ int j___0 ;
  ...
  while (j___0 < numf1s) {
    su += *(f1_layer_U + j___0);
    sp += *(f1_layer_P + j___0);
    su2 += *(f1_layer_U + j___0) * *(f1_layer_U + j___0);
    sp2 += *(f1_layer_P + j___0) * *(f1_layer_P + j___0);
    sup += *(f1_layer_U + j___0) * *(f1_layer_P + j___0);
    j___0 ++;
  }
...
{ int j___0 ;
  ...
  while (! matched) {
    ...
    while (1) {
      ...
      ti = 0;
      while (ti < numf1s) {
        *(f1_layer_U + ti) = *(f1_layer_I + (cp * (unsigned int )numf1s + ti)) + a * *(f1_layer_U + ti);
        tnorm += *(f1_layer_U + ti) * *(f1_layer_U + ti);
        ti ++;
      }
      tnorm = sqrt(tnorm);
      tj = 0;
      ...
      ti = 0;
      while (ti < numf1s) {
        if (*(f1_layer_U + ti) < theta) {
          xr = (double )0;
        } else {
          xr = *(f1_layer_U + ti);
        }
        if (*(f1_layer_P + ti) < theta) {
          qr = (double )0;
        } else {
          qr = *(f1_layer_P + ti);
        }
        *(f1_layer_U + ti) = xr + b * qr;
        tnorm += *(f1_layer_U + ti) * *(f1_layer_U + ti);
        ti ++;
      }
      tnorm = sqrt(tnorm);
      ...
      while (ti < numf1s) {
        ...
        *(f1_layer_P + ti) = *(f1_layer_U + ti) + tsum;
      ...
      while (tj < numf2s) {
        ...
        if (! (Y + tj)->reset) {
          ti = 0;
          while (ti < numf1s) {
            (Y + tj)->y += *(f1_layer_P + ti) * *(bus + (tj * (unsigned int )numf1s + ti));
            ti ++;
          }
        }
        tj ++;
